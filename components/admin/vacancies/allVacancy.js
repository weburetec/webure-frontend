...
import { filterVacancy } from "../../../feature/reducer/vacancy/vacanciesReducer";
...
export const columns = [
  ...
  {
    name: "Open For",
    selector: (row) => row.openVacancy + " People",
  },
  {
    name: "Actions",
...
const AllVacancy = () => {
  ...
  const { vacancies } = useSelector((s) => s.vacancyReducer);
  const router = useRouter();
  const inputRef = useRef("");

  const filterHandler = (e) => {
    e.preventDefault();
    dispatch(filterVacancy(inputRef.current.value));
  };
          ...
          <Menu.Item>
            {({ active }) => (
              <div
                ...
                onClick={() => router.push(`/admin/vacancy/${row._id}`)}
              >
                ...
                onClick={() =>
                  Swal.fire({
                    ...
                    if (result.isConfirmed) {
                      dispatch(deleteVacancyById(row._id));
                    }
                  })
                }
              >
          ...
          <Menu.Item>
            {({ active }) => (
              <div
                ...
                onClick={() => router.push(`/admin/vacancy/edit/${row._id}`)}
              >
                <div className="flex text-center justify-center gap-2 font-semibold">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
